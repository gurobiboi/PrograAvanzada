...
class DCCrewmates(Thread):
    ...
    def run(self):
        ...
        # Se comienza thread de cada tripulante
        for tripulante in self.tripulantes:
            tripulante.start()
        # Se comienza el thread de impostor
        self.impostor.start()

        # Se espera a que terminen los thread de los tripulantes
        for tripulante in self.tripulantes:
            tripulante.join()
        # Se activa evento de termino
        self.evento_termino.set()
        # Se espera a que termine el impostor
        self.impostor.join()
        ganador = "Tripulantes"
        for tarea in self.diccionario_tareas:
            if not self.diccionario_tareas[tarea]["realizado"]:
                ganador = 'Impostor'
                break

        if ganador == "Tripulantes":
            print("Ganan los Tripulantes!")
        else:
            print("Gana el impostor !")

        # Color del impostor
        print(f"El color del impostor era: {self.impostor.color}")
