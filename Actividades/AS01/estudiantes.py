...
class Estudiante(Programago):
    ...
    @abstractmethod
    def __init__(self, nombre, saludo, *args):
        super().__init__(nombre, saludo, *args)
        self.nombre = nombre
        self.saludos = saludo
        self.valor = 10
...
class EstudianteGryffindor(Estudiante):
    ...
    def __init__(self, nombre, saludo, *args):
        super().__init__(nombre, saludo, *args)
        self.valor += random.randint(BONOS_CASA['MIN_VALOR_GRYFFINDOR'],
                                     BONOS_CASA['MAX_VALOR_GRYFFINDOR'])
...
class EstudianteSlytherin(Estudiante):
    ...
    def __init__(self, nombre, saludo, *args):
        super().__init__(nombre, saludo, *args)
        self.ambicion += random.randint(BONOS_CASA['MIN_AMBICION_SLYTHERIN'],
                                        BONOS_CASA['MAX_AMBICION_SLYTHERIN'])
