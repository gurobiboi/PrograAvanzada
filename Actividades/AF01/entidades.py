class Criatura:
    ...
    def preferencia_combate(self):
        if len(self.tipo) > 5:
            self.preferencia = "Especial"
        else:
            self.preferencia = "Fisico"
        return self.preferencia

    def recibir_ataque(self, dano):
        # print(f"{self.nombre} ha recibido {dano} de dano!!")
        self.hp -= dano

    def __str__(self):
        return "Criatura: {self.nombre}, tipo: {self.tipo}".format(self=self)
